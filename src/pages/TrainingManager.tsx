...
export default function TrainingManager({ defaultViewMode = 'list' }: { defaultViewMode?: 'list' | 'calendar' }) {
  ...
  const handleNotifyAttendees = async () => {
    ...
    try {
      const res = await fetch(`/api/trainings/${selectedTrainingId}/notify`, {
        method: 'POST'
      });
      const data = await res.json();
      if (data.success) {
        alert(`Successfully sent ${data.sentCount} reminder email(s) to planned attendees.`);
      } else {
      ...
      <div className="max-w-[1400px] mx-auto pb-12">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleNotifyAttendees} className="px-3 py-1.5 bg-white border border-gray-200 text-gray-700 text-xs font-semibold rounded shadow-sm hover:bg-gray-50 flex items-center gap-2 transition-colors">
              <Mail size={14} /> Notify attendees
            </button>
          </div>
        </div>
    ...
    <div className="max-w-[1400px] mx-auto pb-12">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <h2 className="text-3xl font-semibold text-gray-900">Training Manager</h2>
